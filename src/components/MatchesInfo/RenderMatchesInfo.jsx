...
function MatchesInfoSummoners() {
  ...
  const handleToggle = (participantId) => {
    if (expandedParticipants.includes(participantId)) {
      setExpandedParticipants((prev) => prev.filter((id) => id !== participantId));
    } else {
      setExpandedParticipants((prev) => [...prev, participantId]);
    }
  };
      ...
      {historyMatchGames.map((data) => {
        ...
        return (
          <section key={info.gameId}>
            ...
            <div className="div-win-o-lose">
              <h3 className={winFirstGroup === true ? 'win-color' : 'lose-color'}>
                {winFirstGroup === true ? 'VICTORIA' : 'DERROTA'}
              </h3>

              <h3 className={winSecondGroup === true ? 'win-color' : 'lose-color'}>
                {winSecondGroup === false ? 'DERROTA' : 'VICTORIA'}
              </h3>
            </div>
            <section className="teamskills-section">
              {teams.map((data, indexTemporal) => {
                ...
                return (
                  <section className="section-images-teamskill" key={indexTemporal}>
                    <div className="image-baron">
                      <img src="/kill.svg" alt="Asesinatos" title="Asesinatos"></img>
                      <p> {objectives.champion.kills}</p>
                    </div>
                    <div className="image-baron">
                      <img src="/baron.svg" alt="Baron" title="Baron"></img>
                    ...
                    <div className="image-drake">
                      <img src="/dragon.svg" alt="Dragones" title="Dragones"></img>
                      <p> {objectives.dragon.kills}</p>
                    </div>
                    <div className="image-drake">
                      <img src="/tower.svg" alt="Torres" title="Torres"></img>
